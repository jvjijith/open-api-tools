...
export default function ExplorerPanel() {
  ...
  const [hovered, setHovered] = React.useState<HoveredState | null>(null);
  ...
  // Compute tooltip position: prefer right side, fall back to left if near screen edge
  const getTooltipStyle = (rect: DOMRect): React.CSSProperties => {
    const tooltipWidth = 380;
    const spaceRight = window.innerWidth - rect.right;
    const left = spaceRight >= tooltipWidth + 10
      ? rect.right + 8
  ...
  return (
    <>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5">
              ...
              <div key={tag} className="space-y-2">
                ...
                >
                  <span className="text-xs font-bold text-foreground/80 flex items-center gap-2">
                    <Folder
                      size={12}
                      className={`text-primary/75 transition-transform duration-150 ${
                    ...
                    {eps.map((ep) => {
                      ...
                      return (
                        <div
                          ...
                          onMouseEnter={(e) => {
                            setHovered({ ep, rect: e.currentTarget.getBoundingClientRect() });
                          }}
                          onMouseLeave={() => setHovered(null)}
                          className={`relative flex items-center gap-3 p-2.5 rounded-lg border cursor-pointer select-none transition-all duration-150 hover:scale-[1.005] ${
                            isSelected
                        ...
                        >
                          ...
                          <div className="shrink-0 text-primary">
                            {isSelected ? (
                              <CheckSquare size={13} className="fill-primary/15" />
                            ) : (
                              <Square size={13} />
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0 pr-1">
                            <div className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1.5 shrink-0 pl-2">
                                {spec && (
                                  <span
                                    className={`px-1 text-[7.5px] border rounded font-mono font-black ${
          ...
          {allEndpoints.length > 0 && filteredEndpoints.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
              ...
              <p className="text-[10px] opacity-70 mt-0.5">
                Try widening your search terms or clearing method filters.
              </p>
            </div>
          )}
          ...
          {allEndpoints.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center text-muted-foreground">
              ...
              <p className="text-[10px] opacity-70 mt-0.5">
                Please upload or fetch specifications in the Sidebar.
              </p>
            </div>
          )}
        </div>
      ...
      {/* Portal tooltip — renders at document.body level to escape overflow clipping */}
      {hovered &&
        createPortal(
          <div
            className="bg-[#090d1a]/97 backdrop-blur-md border border-indigo-500/20 rounded-xl p-4 shadow-2xl text-left animate-fadeIn"
            style={getTooltipStyle(hovered.rect)}
          >
            <div className="max-h-[480px] overflow-y-auto pr-1.5 space-y-3.5 scrollbar-thin scrollbar-thumb-primary/20 scrollbar-track-transparent">
              {/* Title */}
              <div className="flex items-center gap-1.5 border-b border-border/40 pb-2 font-mono">
                <span
                  className={`text-[8.5px] font-black tracking-wider uppercase px-1.5 py-0.5 border rounded ${
                    (METHOD_COLORS[hovered.ep.method.toLowerCase()] || METHOD_COLORS.get).bg
                  } ${(METHOD_COLORS[hovered.ep.method.toLowerCase()] || METHOD_COLORS.get).text} ${
                    (METHOD_COLORS[hovered.ep.method.toLowerCase()] || METHOD_COLORS.get).border
                  }`}
                >
                  {hovered.ep.method}
                </span>
                <span
                  className="text-[10px] font-bold text-slate-200 break-all"
                  title={hovered.ep.path}
                >
                  {hovered.ep.path}
                </span>
              </div>
              ...
              {/* Summary & Description */}
              {(hovered.ep.summary || hovered.ep.description) && (
                <div className="space-y-1">
                  {hovered.ep.summary && (
                    <div className="text-[9.5px] font-bold text-slate-100">{hovered.ep.summary}</div>
                  )}
                  {hovered.ep.description && (
              ...
              <div className="space-y-1.5">
                ...
                {hovered.ep.parameters && hovered.ep.parameters.length > 0 ? (
                  <div className="space-y-2 font-mono text-[8.5px]">
                    {['path', 'query', 'header', 'cookie', 'body', 'formData'].map((inType) => {
                      const params = (hovered.ep.parameters || []).filter(
                        (p: any) => (p.in || 'query') === inType
                      );
                      if (params.length === 0) return null;
                      return (
                        <div key={inType} className="space-y-1">
                          <span className="text-[7.5px] font-bold text-indigo-400/90 uppercase block tracking-wider">
                            {inType}
                          </span>
                          <div className="space-y-1">
                            {params.map((p: any, idx: number) => {
                              ...
                              return (
                                <div
                                  key={idx}
                                  className="bg-secondary/20 p-1.5 rounded border border-border/10"
                                >
                                  <div className="flex justify-between items-start gap-2">
                                    ...
                                    <span className="text-muted-foreground text-[7.5px] shrink-0">
                                      ({typeStr})
                                    </span>
                                  </div>
                                  {p.description && (
                  ...
                  <span className="text-[8.5px] text-muted-foreground italic font-mono block">
                    None
                  </span>
                )}
              </div>
                ...
                <div className="space-y-1.5 border-t border-border/20 pt-2">
                  ...
                  <div className="text-[8.5px] font-mono text-slate-300 space-y-1">
                    <div className="flex justify-between items-center">
                      ...
                      <span
                        className={
                          hovered.ep.requestBody.required
                            ? 'text-rose-400 font-bold'
                            : 'text-slate-500'
                        }
                      >
                        {hovered.ep.requestBody.required ? 'Required' : 'Optional'}
                      </span>
                    </div>
                    {hovered.ep.requestBody.description && (
                      <p className="text-[8px] text-muted-foreground leading-normal whitespace-pre-wrap">
                        {hovered.ep.requestBody.description}
                      </p>
                    )}
                    {hovered.ep.requestBody.content && (
                      <div className="space-y-0.5 pt-0.5 border-t border-border/5">
                        {Object.entries(hovered.ep.requestBody.content).map(
                          ([contentType, mediaTypeObj]: [string, any]) => {
                            const schemaRef =
                              mediaTypeObj.schema?.$ref?.split('/').pop() ||
                              mediaTypeObj.schema?.type ||
                              'any';
                            return (
                              <div
                                key={contentType}
                                className="flex justify-between items-center text-[7.5px] text-muted-foreground"
                              >
              ...
              <div className="space-y-1.5 border-t border-border/20 pt-2">
                ...
                {hovered.ep.responses ? (
                  <div className="space-y-1.5 font-mono text-[8.5px]">
                    {Object.entries(hovered.ep.responses).map(([code, r]: [string, any]) => {
                      let responseSchema = '';
                      if (r.content) {
                        const firstContent = Object.values(r.content)[0] as any;
                        if (firstContent?.schema) {
                          responseSchema =
                            firstContent.schema.$ref?.split('/').pop() ||
                            firstContent.schema.type ||
                            '';
                        }
                      } else if (r.schema) {
                        responseSchema =
                          r.schema.$ref?.split('/').pop() || r.schema.type || '';
                      }
                      return (
                        <div
                          key={code}
                          className="flex flex-col text-slate-300 bg-secondary/10 p-1.5 rounded border border-border/10 gap-0.5"
                        >
                          <div className="flex justify-between items-center">
                            <span
                              className={`font-bold ${
                            ...
                            {responseSchema && (
                              <span className="text-indigo-400 text-[7.5px] font-semibold">
                                {responseSchema}
                              </span>
                            )}
                          </div>
              ...
              {/* Security */}
              {hovered.ep.security && hovered.ep.security.length > 0 && (
                <div className="space-y-1.5 border-t border-border/20 pt-2">
                  <span className="text-[8px] font-bold text-muted-foreground uppercase font-mono block tracking-wider">
                    Security Schemes
                  </span>
                  <div className="flex flex-wrap gap-1 pt-0.5">
                    {hovered.ep.security
                      .flatMap((secObj: any) => Object.keys(secObj))
                      .map((scheme, idx) => (
                        <span
                          key={idx}
                          className="bg-purple-500/10 border border-purple-500/25 text-purple-300 text-[8px] font-mono px-2 py-0.5 rounded font-semibold"
                        >
                          {scheme}
                        </span>
                      ))}
